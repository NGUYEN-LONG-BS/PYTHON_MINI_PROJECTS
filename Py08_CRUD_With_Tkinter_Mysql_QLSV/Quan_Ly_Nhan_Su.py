...
Label(root, text='ÚNG DỤNG QUẢN LÝ NHÂN SỰ',fg='red', font=('cambria',16),width=25).grid(row=0) 
listbox=Listbox(root, width=80,height=20)
listbox.grid(row=1, columnspan=2)
show()
Label(root, text='Mã NV:').grid(row=2, column=0)
Entry(root, width=30, textvariable=id).grid(row=2, column=1)
Label(root, text='Tên Nhân viên:').grid(row=3, column=0)

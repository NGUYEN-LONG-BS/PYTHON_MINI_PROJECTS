...
# Hàm để tạo tên file ngắn hơn
def shorten_file_name(file_path):
    # Cắt bớt phần tên file nếu tên quá dài
    base, ext = os.path.splitext(file_path)
    max_length = 200  # Giới hạn độ dài tối đa của tên file (bao gồm cả phần mở rộng)
    if len(base) > max_length:
        base = base[:max_length]  # Cắt bớt tên file
    return base + ext
...
def process_pdfs():
    ...
    if folder_path:
        ...
        if destination_folder:
            # Duyệt qua các thư mục con trong thư mục đã chọn (bao gồm thư mục con sâu hơn)
            for subdir, _, files in os.walk(folder_path):
                for filename in files:
                    if filename.endswith(".pdf"):
                        pdf_path = os.path.join(subdir, filename)
                        
                        # Tạo tên file mới ngắn hơn
                        output_pdf_path = shorten_file_name(os.path.join(subdir, f"modified_{filename}"))

...
# Hàm gộp giá trị các ô trong mỗi dòng thành một chuỗi
def merge_values_into_string(data_df):
    merged_data = []
    for index, row in data_df.iterrows():
        # Gộp tất cả giá trị trong dòng thành một chuỗi, ngăn cách bằng dấu cách hoặc dấu phân cách khác
        merged_string = ' '.join(str(value) for value in row)

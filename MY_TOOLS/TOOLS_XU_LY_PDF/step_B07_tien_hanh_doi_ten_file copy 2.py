...
def process_files():
    ...
    for index, row in df.iterrows():
        ...
        updated_name = str(row.iloc[1])  # Chuyển updated_name sang chuỗi

        # Kiểm tra xem tên file có hợp lệ hay không
        if not isinstance(file_name, str) or not isinstance(updated_name, str):
            updated_files.append(f"Thứ tự {index + 1}: Tên file không hợp lệ!")
            continue

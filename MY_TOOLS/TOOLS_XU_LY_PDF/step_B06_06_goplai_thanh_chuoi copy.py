...
# Hàm gộp giá trị các ô trong mỗi dòng thành một chuỗi (tránh khoảng trắng thừa)
def merge_values_into_string(data_df):
    merged_data = []
    for index, row in data_df.iterrows():
        # Gộp tất cả giá trị trong dòng thành một chuỗi, không có khoảng trắng thừa
        merged_string = ' '.join(str(value) if pd.notna(value) else '' for value in row)  # Không thêm khoảng trắng nếu ô rỗng
        merged_data.append([merged_string])  # Mỗi chuỗi là một danh sách

    return pd.DataFrame(merged_data)

# Hàm xuất kết quả đổi tên và lưu
...
# Hàm xuất kết quả gộp các ô thành chuỗi
def save_merged_data():
    # Chọn file dữ liệu cần gộp
    data_file = filedialog.askopenfilename(title="Chọn file dữ liệu để gộp", filetypes=[("Excel files", "*.xlsx;*.xls")])
    if not data_file:
        return  # Nếu người dùng không chọn file, dừng chương trình
    ...
    # Gộp các ô trong mỗi dòng thành một chuỗi
    merged_df = merge_values_into_string(data_df)
    ...
    # Lưu kết quả vào file Excel mới
    try:
        merged_df.to_excel(output_file, index=False, header=False)  # Không ghi tiêu đề cột
        messagebox.showinfo("Merged Files", f"File has been saved to {output_file}")
    except Exception as e:
        messagebox.showerror("Error", f"Failed to save merged data: {str(e)}")
...
# Nút để chọn và gộp các ô thành chuỗi
merge_button = tk.Button(root, text="Choose Data File and Merge Values", command=save_merged_data)

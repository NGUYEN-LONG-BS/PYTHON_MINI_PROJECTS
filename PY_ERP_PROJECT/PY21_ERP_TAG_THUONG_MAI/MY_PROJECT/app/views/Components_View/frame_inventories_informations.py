...
class cls_frame_inventories_information_model:
    ...
    def get_data(query=''):
        ...
        #             ELSE FORMAT([TONG_SL_TON], 'N2')  -- Nếu có phần thập phân, hiển thị 2 chữ số
        #         END AS [SL khả dụng]
        #     FROM [{database_name}].[dbo].[VIEW_INVENTORY_REPORT_QUANTITY]
        #     ORDER BY [MA_HANG]
        #     """
        ...
        query = f"""
            ...
            FROM 
                [{database_name}].[dbo].[VIEW_INVENTORY_REPORT_QUANTITY] inv_table
            LEFT JOIN 
                [TBD_2024].[dbo].[VIEW_INVENTORY_REPORT_STANDAR] report_table
...
class cls_TreeviewCombobox_inventories(entry.cls_my_text_entry_num_01):
    ...
    def f_show_dropdown(self, event=None):
        if not self.dropdown:
            ...
            self.tree = ttk.Treeview(self.dropdown, columns=self.columns, show="headings", name="treeview_toplevel_inventories_dropdown")
